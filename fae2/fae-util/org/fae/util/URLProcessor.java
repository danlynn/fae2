...
public class URLProcessor {
    ...
    if ((depth + 1) <= faeUtil.DEPTH) {
      ...
      for (String link : links) {
        ...
        if (authorizationUrlMatch(faeUtil, urlNum, cnt, link)) {
          if (traverseIt(faeUtil, url, depth, urlNum, cnt, m_loggedInURL))
            traverseURL(faeUtil, url, m_loggedInURL, depth + 1, newBase);
   ...
   * @throws Exception 
   */
  private boolean authorizationUrlMatch(FaeUtil faeUtil, int urlNum, int cnt, String url)
    throws Exception {
    ...
    for(String tmp: faeUtil.getAuthorizationURLs()){
	    ...
	    String matchType = (String) (xpath.compile("//authorization[url='" + tmp + "']/url-match/@type")).evaluate(doc, XPathConstants.STRING);
	    
	    if(matchType.equals("endsWith")){
	    	if(url.endsWith(tmp) && !m_alreadyProcessedAuthorizationUrls.contains(tmp)){
		    	urlMatch = true;
		    	matcher = tmp;
	    	...
	    	if(url.startsWith(tmp) && !m_alreadyProcessedAuthorizationUrls.contains(tmp)){
		    	urlMatch = true;
		    	matcher = tmp;
		        m_alreadyProcessedAuthorizationUrls.add(tmp);
		        break;
	    	...
	    	if(url.contains(tmp) && !m_alreadyProcessedAuthorizationUrls.contains(tmp)){
		    	urlMatch = true;
		    	matcher = tmp;
		        m_alreadyProcessedAuthorizationUrls.add(tmp);
		        break;
	    ...
	    else if (faeUtil.getAuthorizationURLs().contains(url) && !m_alreadyProcessedAuthorizationUrls.contains(url)) {
	      urlMatch = true;
    ...
    if (urlMatch) {
      //System.out.println("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
      System.out.println(urlNum + ":" + cnt + ": AUTHORIZATION URL MATCH: " + url);
   ...
   * @throws Exception
   */
  public void processLogin(FaeUtil faeUtil, int urlNum, int cnt, String url, String matcher)
    throws Exception {
    Document doc = faeUtil.getAuthorizationDoc();
    XPath xpath = faeUtil.getAuthorizationXpath();

    // Extract xml data based on url
    String submitName = (String) (xpath.compile("//authorization[url='" + matcher + "']/submit/@name")).evaluate(doc, XPathConstants.STRING);
    String submitValue = (String) (xpath.compile("//authorization[url='" + matcher + "']/submit/@value")).evaluate(doc, XPathConstants.STRING);
    String submitTag = (String) (xpath.compile("//authorization[url='" + matcher + "']/submit/@tag")).evaluate(doc, XPathConstants.STRING);
    String submitType = (String) (xpath.compile("//authorization[url='" + matcher + "']/submit/@type")).evaluate(doc, XPathConstants.STRING);
    Double formIndex = (Double) (xpath.compile("//authorization[url='" + matcher + "']/form/@index")).evaluate(doc, XPathConstants.NUMBER);
    String formName = (String) (xpath.compile("//authorization[url='" + matcher + "']/form/@name")).evaluate(doc, XPathConstants.STRING);
    String formId = (String) (xpath.compile("//authorization[url='" + matcher + "']/form/@id")).evaluate(doc, XPathConstants.STRING);
    String formuser = (String) (xpath.compile("//authorization[url='" + matcher + "']/control/@name")).evaluate(doc, XPathConstants.STRING);
    String formpass = (String) (xpath.compile("//authorization[url='" + matcher + "']/control[2]/@name")).evaluate(doc, XPathConstants.STRING);
    String user = (String) (xpath.compile("//authorization[url='" + matcher + "']/control/@value")).evaluate(doc, XPathConstants.STRING);
    String pass = (String) (xpath.compile("//authorization[url='" + matcher + "']/control[2]/@value")).evaluate(doc, XPathConstants.STRING);
    String anchorText = (String) (xpath.compile("//authorization[url='" + matcher + "']/anchor/@text")).evaluate(doc, XPathConstants.STRING);
    String verification = (String) (xpath.compile("//authorization[url='" + matcher + "']/verification/@enabled")).evaluate(doc, XPathConstants.STRING);
    String verificationText = (String) (xpath.compile("//authorization[url='" + matcher + "']/verification/@text")).evaluate(doc, XPathConstants.STRING);
    Double controlCount = (Double) (xpath.compile("//authorization[url='" + matcher + "']/control/@count")).evaluate(doc, XPathConstants.NUMBER);
   ...
   HtmlPage page1 = webClient.getPage(url);
    ...
    if(!controlCount.toString().equals("NaN")){
    	...
    	for(int i=1; i<=controlCount.intValue();i++){
    			String controlName = (String) (String) (xpath.compile("//authorization[url='" + matcher + "']/control[@id='"+i+"']/@name")).evaluate(doc, XPathConstants.STRING);
	    		String controlValue = (String) (xpath.compile("//authorization[url='" + matcher + "']/control[@id='"+i+"']/@value")).evaluate(doc, XPathConstants.STRING);
	    		form.getInputByName(controlName).setValueAttribute(controlValue);    		
    	}
    }	
    else{
    ...
    /*HtmlPage page2;
    if(!submit.isEmpty())
    	page2 = (HtmlPage) form1.getInputByValue(submit).click();
    else*/
    HtmlPage page2 = null; 
    if (!submitName.isEmpty())
    	...
    	page2 = (HtmlPage) form.getElementsByAttribute(submitTag, "type", submitType).get(0).click();

    m_loggedInURL = page2.getUrl().toString();
    
    if(!verification.isEmpty() && verification.equals("true")){
	    if (!verificationText.isEmpty() && page2.asText().contains(verificationText) && !user.isEmpty() && !pass.isEmpty()) {
	      System.out.println(urlNum + ":" + cnt + ": LOGIN SUCCESSFUL");
	      m_loginSuccessURLs.add(url);
	    }
	    else if (!verificationText.isEmpty() && page2.asXml().contains(verificationText) && !user.isEmpty() && !pass.isEmpty()) {
	        System.out.println(urlNum + ":" + cnt + ": LOGIN SUCCESSFUL");
	        m_loginSuccessURLs.add(url);
	      }
	    else {
	      System.err.println(urlNum + ":" + cnt + ": LOGIN FAILED WITH SUPPLIED CREDENTIALS IN authorization.xml!");
   ...
   * @throws Exception
   */
  public static void imageLinks(HtmlPage page)
    throws Exception {
  ...
  public void process(FaeUtil faeUtil, String urlFrom, String url) {
    ...
    try {
      // ------------------------------------------------------------------------
      // ------------------------------------------------------------------------
      // make a WebClient for each URL - DON"T DO THIS NOW SINCE LOGGING IN LOGIC ADDED!
  ...
  public static class Processor implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        //        for (int i = 0; i < window.size(); i++) {// page within the frames
